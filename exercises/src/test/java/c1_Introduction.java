...
public class c1_Introduction extends IntroductionBase {
    ...
    public void nothing_happens_until_you_() throws InterruptedException {
        ...
        // Assertion happen in main thread and subscribe happen in another thread, we need to slow down main thread until scheduler thread done with updating list
    ...
    public void leaving_blocking_world_behind() throws InterruptedException {
        ...
        fortuneTop5().doOnNext(companyList::add).doOnError(throwable -> {}).doOnComplete(() -> serviceCallCompleted.set(true)).subscribe();
